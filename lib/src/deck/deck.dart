...
typedef OnSlideDeckWidgetUpdate = void Function(SlideDeck old, SlideDeck current);
...
class SlideDeck extends StatefulWidget {
  ...
  const SlideDeck({
    ...
    this.includeDeckControls = true,
    this.onSlideDeckWidgetUpdate = _onSlideDeckWidgetUpdateDoNothingElse,
    this.fineRewind = false,
  ...
  final bool includeDeckControls;

  final OnSlideDeckWidgetUpdate onSlideDeckWidgetUpdate;

  /// If true the previous action will iterate over sub indexes, otherwise jump to previous index
  final bool fineRewind;
  ...
  static void _onSlideDeckWidgetUpdateDoNothingElse(SlideDeck old, SlideDeck current) {}
}
...
class SlideDeckState extends State<SlideDeck> {
  ...
  Widget build(BuildContext context) {
    ...
    var slide = AspectRatio(
      ...
      child: Stack(
        ...
        children: [
          ...
          if (widget.includeDeckControls && !widget.autoplay && !_presenterView)
            Positioned(
              bottom: 16.0,
              right: 16.0,
              child: MouseRegion(
    ...
    return !widget.includeDeckControls ? content : Focus(
